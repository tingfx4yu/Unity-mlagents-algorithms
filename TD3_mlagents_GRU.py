...
class replay_memory:
    def __init__(self,capacity):
        ...
        self.Experience = namedtuple('Experience',['h_in','h_out','state','ir_state','action','last_action','next_state','next_ir_state','reward','done'])
    
    def add(self,h_in,h_out,state,ir_state,action,last_action,next_state,next_ir_state,reward,done):
        e = self.Experience(h_in,h_out,state,ir_state,action,last_action,next_state,next_ir_state,reward,done)
    ...
    def sample(self,batch_size):
        ...
        return hidden_in,hidden_out,states,ir_states,actions,last_actions,next_states,next_ir_states,rewards,dones
    def __len__(self):
        return len(self.memory)

class Actor(nn.Module):
    def __init__(self,state_dim,rnn_dim,act_dim):
        super(Actor,self).__init__()
        self.f1 = nn.Linear(state_dim, hidden_layer)
        self.f1_2 = nn.Linear(rnn_dim + act_dim,rnn_layer)
        self.gru1 = nn.GRU(rnn_layer,rnn_layer)
        self.f3 = nn.Linear(hidden_layer+rnn_layer,hidden_layer)
        self.f4 = nn.Linear(hidden_layer,act_dim)
        self.max_action = 0.56

    def forward(self,state,ir_state,last_action,hidden_in):
        state = state.permute(1,0,2)
        ir_state = ir_state.permute(1,0,2)
        last_action = last_action.permute(1,0,2)
        fc_branch = F.relu(self.f1(state))
        fc_branch = fc_branch.permute(1,0,2)
        gru_branch = torch.cat([ir_state,last_action],-1)
    ...
    def evaluate(self,state,ir_state,last_action,hidden_in,noise_scale = 0.0):
        #normal = Normal(0,1)
        action, hidden_out = self.forward(state,ir_state,last_action,hidden_in)
...
class Critic(nn.Module):
    def __init__(self,state_dim,rnn_dim,act_dim):
        ...
        self.f1_2 = nn.Linear(rnn_dim+act_dim,rnn_layer)
        self.gru1 = nn.GRU(rnn_layer,rnn_layer)
        self.f2 = nn.Linear(hidden_layer+rnn_layer,hidden_layer)
        ...
        self.l1_2 = nn.Linear(rnn_dim + act_dim,rnn_layer)
        #self.gru2 = nn.GRU(rnn_layer,rnn_layer)
        self.l2 = nn.Linear(hidden_layer+rnn_layer,hidden_layer)
        self.l3 = nn.Linear(hidden_layer,hidden_layer)
        self.l4 = nn.Linear(hidden_layer,1)
    def forward(self,state,ir_state,action,last_action,hidden_in):
        state = state.permute(1,0,2)
        ir_state = ir_state.permute(1,0,2)
        last_action = last_action.permute(1,0,2)
        ...
        gru_branch_2, gru_hidden_2 = self.gru1(gru_branch_2,hidden_in)
    ...
    def Q1(self,state,ir_state,action,last_action,hidden_in):
        state = state.permute(1,0,2)
        ir_state = ir_state.permute(1,0,2)
        last_action = last_action.permute(1,0,2)
        fc_branch = torch.cat([state,action],-1)
...
class Critic(nn.Module):
    def __init__(self,state_dim,rnn_dim,act_dim):
        super(Critic,self).__init__()
        self.f1 = nn.Linear(act_dim+state_dim+rnn_dim,512)
        self.f2 = nn.Linear(512,256)
        self.f3 = nn.Linear(256,256)
        self.f4 = nn.Linear(256,256)
        self.f5 = nn.Linear(256,1)

        self.l1 = nn.Linear(act_dim+state_dim+rnn_dim,512)
    ...
    def forward(self,state,ir_state,action):
        ...
        return q1,q2
    def Q1(self,state,ir_state,action):
        sa = torch.cat([state,ir_state,action],1)

        q1 = F.gelu(self.f1(sa))
        q1 = F.gelu(self.f2(q1))
        q1 = F.gelu(self.f3(q1))
        q1 = F.gelu(self.f4(q1))
        q1 = self.f5(q1)
...
class TD3():
    def __init__(self):
        ...
        self.actor = Actor(state_dim,rnn_dim,act_dim).cuda()
        self.actor_target = copy.deepcopy(self.actor)
        self.critic = Critic(state_dim,rnn_dim,act_dim).cuda()
    ...
    def select_action(self,states,ir_states,last_action,hidden_in):
        actions = np.zeros((num_agents,act_dim),dtype = 'float')
        last_action = torch.from_numpy(last_action).cuda().float()
        hidden_out = torch.zeros(num_agents,1,1,rnn_layer).cuda().float()
        ...
        for i in range(num_agents):
            ...
            if self.load == True:
                with torch.no_grad():
                    ...
                    #state = states[i,:]
                    act, hid_out = self.actor.forward(state,ir_state,last_act,hidden_in_1)
    ...
    def update(self):
        ...
        hidden_in,hidden_out,state,ir_state,action,last_action,next_state,next_ir_state,reward,done = self.memory.sample(batch_size)
        self.total += 1
        ...
        #noise = (torch.randn_like(action)*self.policy_noise).clamp(-self.noise_clip,self.noise_clip)
            new_action, _ = self.actor.evaluate(state,ir_state,last_action,hidden_in)
            #new_action = new_action.squeeze(1)

            new_next_action,_ = self.actor_target.evaluate(next_state,next_ir_state,action,hidden_out)
            #next_action = next_action.squeeze(1)

            target_Q1, target_Q2 = self.critic_target(next_state.squeeze(0),next_ir_state.squeeze(0),new_next_action.squeeze(1))
            target_Q = torch.min(target_Q1,target_Q2)*0.8 + torch.max(target_Q1,target_Q2)*0.2
            target_Q = reward + ((1-done*GAMMA*target_Q))

        #get current Q

        current_Q1, current_Q2 = self.critic(state.squeeze(0),ir_state.squeeze(0),action.squeeze(0))
        ...
        if self.total % self.policy_freq == 0:      
            #compute actor loss
            actor_loss =  self.critic.Q1(state.squeeze(0),ir_state.squeeze(0),new_action.squeeze(1))
    ...
    def step(self,hidden_in,hidden_out,state,ir_state,action,last_action,next_state,next_ir_state,reward,done):
        if self.load == True:
            return
        #hidden_in_1 = hidden_in[0]
        #hidden_in_2 = hidden_in[1]
        ...
        for i in range(num_agents):
            ...
            self.memory.add(hidden_in[i].cpu().numpy(),hidden_out[i].cpu().numpy(),state[i],ir_state[i],action[i],last_action[i],next_state[i],next_ir_state[i],reward[i],done[i])
        self.update()
