env_name = "env/Crawler/Crawler_2"
train_model = True
...
for eps in range(1):
    ...
    while True:
        action = naf.select_action(state)
        env_info = env.step(action)[default_brain]
        next_state = env_info.vector_observations
        reward = env_info.rewards
        done = env_info.local_done
        naf.step(state,action,next_state,reward,done)
        state = next_state
        score += reward[0]
        rewards.append(score)
        if naf.steps % 1000000 == 0:
            with open('data_2',mode = 'w') as f:
                for x in rewards:
                    f.write(str(x))
                    f.write('\n')
                rewards.clear()
                f.close()
        print ('step:',naf.steps)
        print("Score:",score)
    '''
    while True:
