...
for eps in range(1):
    ...
    while True:
        ...
        td3.step(state,action,next_state,reward,done)
        state = next_state
        score += reward[0]
        rewards.append(score)
        if td3.steps  == 200000:
            torch.save(td3.actor.state_dict(),'actor_1118_200k.pkl')
            torch.save(td3.critic.state_dict(),'critic_1118_200k.pkl')
        if td3.steps  == 500000:
            torch.save(td3.actor.state_dict(),'actor_1118_500k.pkl')
            torch.save(td3.critic.state_dict(),'critic_1118_500k.pkl')
        if td3.steps == 1000000:
            torch.save(td3.actor.state_dict(),'actor_1118_1m.pkl')
            torch.save(td3.critic.state_dict(),'critic_1118_1m.pkl')    
        if td3.steps % 2000000 == 0:
            with open('data_1124_2m',mode = 'w') as f:
                for x in rewards:
                    f.write(str(x))
                    f.write('\n')
                rewards.clear()
