...
from TD3_mlagents import TD3


id = random.randint(1,10)

#Transition = namedtuple('Transition',['state', 'action', 'reward', 'a_log_prob', 'next_state'])
env = UnityEnvironment(file_name= env_name, worker_id=id, seed =1)
...
for eps in range(1):
    ...
    while True:
        ...
        if save_mode and not td3.load:
            if td3.steps == 250000:
                torch.save(td3.actor.state_dict(),'td3_actor_1221_250k.pkl')
                #torch.save(td3.critic.state_dict(),'td3_critic_1212_180k.pkl')
        
        if td3.steps == 1000000:
            torch.save(td3.actor.state_dict(),'td3_actor_1221_1m.pkl')
